...
<head>
	...
	var rows =null;
		$(function () {
			...
			var cfg = $.extend({}, gridConfig,{
				...
				columns: [
		                	...
		                	{ display: '金额', name: 'money',width:'10%'},
		                	...
		                	{ display: '操作', name: 'userName',width:'5%',render:function(item){
		                		return "<a href='javascript:void(0);' onclick='_openSearch("+item.id+")' ><img title='审核' src='<%=path%>/js/ligerUI/skins/icons/memeber.gif'/></a>";
		                	} }
		                	,
		                	{ display: '操作', name: 'userName',width:'5%',render:function(item){
		                		return "<a href='javascript:void(0);' onclick='_save("+item.id+","+item.workType+","+item.netType+")' ><img title='保存' src='<%=path%>/js/ligerUI/skins/icons/save.gif'/></a>";
		                	} }
		                	</security:authorize>
		                ],
		...
		function _save(id,workType,netType){
			 manager.isDataChanged = false;
			$.ajax({
				url : "<%=path%>/record/editsave_.do",
				data : {"id":id,"workType":workType,"netType":netType},
				type : "post",
				dataType : "json",
				cache : false,
				error : function(XMLHttpRequest, textStatus, errorThrown){
