...
<div  class="m-main">
	...
	<form id="form" method="post" action="<%=path %>/group/group-editsave.do" class="form-horizontal" >
	<input type="hidden" name="id" value="${id }">
	  <div class="control-group">
	    <label class="control-label" for="groupName">组织名</label>
		<div class="controls">
		  <input id="groupName" type="text" name="groupName" value="${group.groupName }"  validate="{required:true,maxlength:15}" >
		  <div></div>
	    </div>
	  </div>
	  ...
	  <div class="control-group">
		...
		<div class="controls">
		  ...
		  <option value="">请选择</option>
		  	<c:forEach items="${list}" var="g" >
		  		<option value="${g.id }" <c:if test="${group.pid == g.id }" >selected</c:if> >${g.groupName }</option>
		  	</c:forEach>
		  </select>
		  <div></div>
	    </div>
		...
		<div class="controls">
		  <select id="type" name="type" validate="{required:true}">
		  	...
		  	<c:forEach items="${dlist}" var="d" >
		  		<option value="${d.value }" <c:if test="${group.type == d.value }" >selected</c:if> >${d.name }</option>
		  	</c:forEach>
		  </select>
		  <div></div>
	    </div>
